import {
  ...
  CambridgeDictionaryEntry,
  CambridgeDefGroup,
  CambridgeDefinition,
  CambridgePronunciation,
  CambridgeExample,
  CambridgeLevel,
  CambridgeRelatedPhrase,
 ...
 */
export class CambridgeDictionaryService extends BaseDictionaryService<CambridgeDictionaryResponse> {
  /**
   * Base URL for Cambridge Dictionary
   * @private
   */
  private readonly baseUrl: string = "https://dictionary.cambridge.org";
  ...
  private handleCambridgeError(
    ...
    return super.handleError(word, error) as CambridgeDictionaryResponse;
  }
  ...
  private extractPronunciation(
    ...
    if (ukIpa) pronunciation.ukIpa = ukIpa;
    if (usIpa) pronunciation.usIpa = usIpa;
    ...
    if (ukAudioElement.length > 0) {
      pronunciation.ukAudioUrl = this.makeAbsoluteUrl(
        ukAudioElement.attr("src")
      );
    }

    if (usAudioElement.length > 0) {
      pronunciation.usAudioUrl = this.makeAbsoluteUrl(
        usAudioElement.attr("src")
      );
    }
  ...
  private extractLevel(
    ...
    if (levelElement.length === 0) return undefined;
  ...
  private extractRelatedPhrases(
    ...
    const phrases: CambridgeRelatedPhrase[] = [];
  ...
  private extractDefinitions(
    ...
    $(element)
      ...
      .each((i, defBlock) => {
        ...
        const definition: CambridgeDefinition = { text };
        ...
        $(defBlock)
          ...
          .each((j, alt) => {
            ...
            if (altText) alternates.push(altText);
          });
        ...
        if (ukVar || usVar) {
          ...
          if (ukVar) definition.regionVariation.uk = ukVar;
          if (usVar) definition.regionVariation.us = usVar;
        }
   ...
   */
  async lookupWord(word: string): Promise<CambridgeDictionaryResponse> {
    try {
      const url = `${this.baseUrl}/dictionary/english/${encodeURIComponent(
        word
      )}`;
      const $ = await super.fetchHtml(url);
      const entries: CambridgeDictionaryEntry[] = [];
      ...
      $(".entry-body__el").each((i, element) => {
        ...
        const entryWord =
          $(element).find(".hw.dhw").first().text().trim() || word;
        ...
        if (definitions.length === 0) return; // Skip if no definitions found

        // Create definition group
        const defGroup: CambridgeDefGroup = {
          partOfSpeech,
          definitions,
        };
        ...
        let entry = entries.find(
          (e) => e.word.toLowerCase() === entryWord.toLowerCase()
        );
        ...
        if (!entry) {
          entry = {
            ...
            defGroups: [],
            pronunciation:
              Object.keys(pronunciation).length > 0 ? pronunciation : undefined,
          };
          entries.push(entry);
        }
        ...
        if (idioms.length > 0) {
          if (!entry.idioms) entry.idioms = [];
          entry.idioms.push(...idioms);
        }

        const phrasalVerbs = this.extractRelatedPhrases(
          $,
          element,
          ".pv-block, .phrasal_verb-block"
        );
        if (phrasalVerbs.length > 0) {
          if (!entry.phrasalVerbs) entry.phrasalVerbs = [];
          entry.phrasalVerbs.push(...phrasalVerbs);
        }

        const relatedPhrases = this.extractRelatedPhrases(
          $,
          element,
          ".phrase-block"
        );
        if (relatedPhrases.length > 0) {
          if (!entry.relatedPhrases) entry.relatedPhrases = [];

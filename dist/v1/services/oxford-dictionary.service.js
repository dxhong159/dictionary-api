...
class OxfordDictionaryService extends base_dictionary_service_1.BaseDictionaryService {
    ...
    async fetchOxfordHtml(word) {
        try {
            ...
            const directSearchUrl = `https://www.oxfordlearnersdictionaries.com/search/english/direct/?q=${encodeURIComponent(word)}`;
    ...
    async lookupWord(word) {
        try {
            ...
            const audioData = {};
            // Find UK audio
            $(".sound.audio_play_button.pron-uk.icon-audio").each((i, element) => {
                if (i === 0) {
            ...
            // Find US audio
            $(".sound.audio_play_button.pron-us.icon-audio").each((i, element) => {
                if (i === 0) {
                    // Take first one for main audio
            ...
            $(".entry").each((i, element) => {
                ...
                const entryUsAudio = $(element)
                    ...
                    .attr("data-src-mp3");
                if (entryUkAudio)
                    entryAudioData.uk = entryUkAudio;
                if (entryUsAudio)
                    entryAudioData.us = entryUsAudio;
                // Process definitions
                ...
                const synonyms = synonymBlock.length > 0
                    ? $(synonymBlock)
                        .text()
                        .trim()
                        .replace("Synonyms:", "")
                ...
                $(element)
                    ...
                    $(idiomGroup)
                        ...
                        idiomDefinitions.push({
                            definition: idiomDef,
                            examples: idiomExamples.length > 0 ? idiomExamples : undefined,
                        });
                    });
            ...
            if (entries.length === 0) {
                const redirectedWord = $(".headword").first().text().trim() || $(".h").first().text().trim();
                if (redirectedWord &&

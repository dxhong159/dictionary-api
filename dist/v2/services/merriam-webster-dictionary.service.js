...
class MerriamWebsterDictionaryService extends base_dictionary_service_1.BaseDictionaryService {
    ...
    makeAbsoluteUrl(relativeUrl) {
        return super.makeAbsoluteUrl("https://www.merriam-webster.com", relativeUrl);
    }
    /**
    ...
    extractPronunciation($, element) {
        ...
        function extractAudioUrl(pronElem, urls, form) {
            ...
            if (audioUrl) {
                ...
                finalUrl = audioUrl;
                console.log(`Found audio URL from data-audio attribute for ${form || "main form"}: ${audioUrl}`);
            }
                ...
                console.log(`Constructed audio URL for ${form || "main form"}: ${finalUrl}`);
            }
            else {
                // Phương pháp 3: Tìm trong thuộc tính onclick
                const onClickAttr = pronElem.attr("onclick");
                if (onClickAttr) {
                    const audioUrlMatch = onClickAttr.match(/(['"])(?:https?:\/\/[^'"]+\.mp3)\1/i);
                    if (audioUrlMatch && audioUrlMatch[1]) {
                        finalUrl = audioUrlMatch[1];
                        console.log(`Extracted audio URL from onclick for ${form || "main form"}: ${finalUrl}`);
                    }
                }
                ...
                if ((hrefAttr || dataUrl) && dataFile && dataDir) {
                    ...
                    const lang = dataLang.replace("_", "/");
                    finalUrl = `https://media.merriam-webster.com/audio/prons/${lang}/mp3/${dataDir}/${dataFile}.mp3`;
                    console.log(`Built audio URL from href/data-url for ${form || "main form"}: ${finalUrl}`);
                }
            }
    ...
    extractEtymology($, element) {
        ...
        const firstUseMatch = etymologyText.match(/first\s+known\s+use\s+in\s+(\d{4})/i);
        if (firstUseMatch) {

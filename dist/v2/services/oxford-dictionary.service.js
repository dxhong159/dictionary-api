...
exports.OxfordDictionaryService = void 0;
 ...
 */
class OxfordDictionaryService extends base_dictionary_service_1.BaseDictionaryService {
    constructor() {
        super("Oxford");
    ...
    makeAbsoluteUrl(relativeUrl) {
        return super.makeAbsoluteUrl("https://www.oxfordlearnersdictionaries.com", relativeUrl);
    }
    /**
     ...
     * @private
     */
    async fetchOxfordHtml(word) {
        try {
            // Use Oxford's direct search URL which automatically redirects to the correct word page
            const directSearchUrl = `${this.baseUrl}/search/english/direct/?q=${encodeURIComponent(word)}`;
            console.log(`Fetching Oxford definition with direct search URL: ${directSearchUrl}`);
            ...
            if ($(".entry").length > 0 || $(".webtop").length > 0) {
                ...
                console.log(`Oxford page doesn't contain expected dictionary content for: ${word}`);
            }
        }
        catch (error) {
        ...
        try {
            const searchUrl = `${this.baseUrl}/search/english/?q=${encodeURIComponent(word)}`;
        ...
        throw new Error(`Could not find Oxford definition for "${word}" using any known URL pattern`);
    }
    /**
     ...
     */ extractPronunciation($, element) {
        ...
        $(element)
            ...
            if (mp3Url) {
                ...
                if (mp3Url) {
                    ...
                    const filenameMatch = mp3Url.match(/\/([a-z]+)(?:__|\w*?)_(?:gb|us)/i);
                ...
                if ($(audioElem).hasClass("pron-uk")) {
                    ...
                    console.log(`UK Audio #${i}: ${absoluteUrl} (${nearestPhonetic || "no phonetic"}) - Form: ${formHint || "unknown"}`);
                }
                    ...
                    console.log(`US Audio #${i}: ${absoluteUrl} (${nearestPhonetic || "no phonetic"}) - Form: ${formHint || "unknown"}`);
                }
                else {
        ...
        $(element)
            ...
            const isUK = phonetic.includes("BrE") ||
                $(phonElem).closest(".pron-g").find(".pron-uk").length > 0;
            const isUS = phonetic.includes("NAmE") ||
            ...
            for (const form of wordForms) {
                if (phonContext.includes(form)) {
                    detectedForm = form;
                    console.log(`Found form ${form} in context of phonetic ${phonetic}`);
                    break;
                }
            }
            ...
            for (const audioArray of audioArrays) {
                if (audioUrl)
                    break; // Nếu đã tìm thấy, dừng tìm kiếm
                if (detectedForm) {
                    const formMatch = audioArray.find((a) => a.formHint === detectedForm);
                    if (formMatch) {
                        audioUrl = formMatch.url;
                        variant = formMatch.variant;
                        console.log(`Matched by word form: ${detectedForm} -> ${audioUrl}`);
                        break;
                    }
                }
            }
            ...
            if (!audioUrl) {
                ...
                if (phonetic.includes("/duː/")) {
                    ...
                    const doForm = audioArrays[0].find((a) => a.formHint === "do" || a.formHint === "does");
                    if (doForm) {
                        audioUrl = doForm.url;
                    ...
                    // Phiên âm cho "doing"
                    const doingForm = audioArrays[0].find((a) => a.formHint === "doing");
                    if (doingForm) {
                        audioUrl = doingForm.url;
                        variant = doingForm.variant;
                        console.log(`Matched by phonetic pattern /ˈduːɪŋ/ -> ${audioUrl}`);
                    }
                }
            }
            ...
            if (!audioUrl) {
                for (const audioArray of audioArrays) {
                    const matchingAudio = audioArray.find((a) => a.phonetic === phonetic);
                    if (matchingAudio) {
                        audioUrl = matchingAudio.url;
                        variant = matchingAudio.variant;
                        console.log(`Matched by exact phonetic: ${phonetic} -> ${audioUrl}`);
                        break;
                    }
                }
            }
            ...
            if (!audioUrl) {
                ...
                if (primaryArray.length > 0) {
                    ...
                    console.log(`Fallback match by position: index ${i} -> ${audioUrl}`);
                }
            }
            ...
            if (!audioUrl) {
                ...
                console.log(`Last resort fallback: using first available audio URL -> ${audioUrl}`);
            }
            ...
            console.log(`Pronunciation: ${phonetic}, Audio: ${audioUrl || "none"}, Variant: ${variant}`);
     ...
     * @private
     */
    extractLexicalCategory($, element) {
     ...
     * @private
     */
    extractGrammaticalFeatures($, element) {
        const features = [];
    ...
    extractEtymology($, element) {
        ...
        const periodMatches = text.match(/(?:Old|Middle|Late|Modern|Ancient)\s+[A-Z][a-z]+/g);
    ...
    async lookupWord(word) {
        try {
            ...
            $(".entry, [hclass='entry']").each((entryIndex, entryElement) => {
                ...
                const posElements = $(entryElement).find("h2.pos-header, span.pos, [hclass='pos']");
                posElements.each((_, posHeaderElement) => {
                    const lexicalCategory = this.extractLexicalCategory($, posHeaderElement);
                    ...
                    // Extract grammatical features
                    const grammaticalFeatures = this.extractGrammaticalFeatures($, entryElement);
                    ...
                    // Extract senses
                    const sensesElement = posContent.length > 0 ? posContent[0] : entryElement;
...
exports.OxfordDictionaryService = OxfordDictionaryService;
//# sourceMappingURL=oxford-dictionary.service.js.map